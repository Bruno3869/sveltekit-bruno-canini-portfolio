<script>

</script>

<div class="container col-xxl-8 px-5 pb-4">
    <footer class="py-4 my-4">
      <div class="d-flex flex-wrap align-items-center justify-content-center justify-content-lg-start">
        <ul class="nav col-12 col-lg-auto me-lg-auto mb-2 justify-content-center mb-md-0 d-none d-lg-flex">
          <li><a href="/" class="nav-link px-2 text-white">Accueil</a></li>
          <li><a href="/experiences" class="nav-link px-2 text-secondary navheader">Expériences</a></li>
          <li><a href="/a-propos" class="nav-link px-2 text-secondary navheader">A propos</a></li>
        </ul>
        <div class="text-end pb-1">
          <span class="gap gap-x-5"><a href="mailto:bruno.canini@gmail.com" class="group cursor-pointer" aria-label="Email"
              title="Email"><svg stroke="currentColor" fill="currentColor" stroke-width="0" viewBox="0 0 24 24"
                height="1em" width="1em" xmlns="http://www.w3.org/2000/svg"
                class="text-xl text-fore-subtle group-hover:-translate-y-2 transition-transform">
                <path fill="none" d="M0 0h24v24H0z"></path>
                <path
                  d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10h5v-2h-5c-4.34 0-8-3.66-8-8s3.66-8 8-8 8 3.66 8 8v1.43c0 .79-.71 1.57-1.5 1.57s-1.5-.78-1.5-1.57V12c0-2.76-2.24-5-5-5s-5 2.24-5 5 2.24 5 5 5c1.38 0 2.64-.56 3.54-1.47.65.89 1.77 1.47 2.96 1.47 1.97 0 3.5-1.6 3.5-3.57V12c0-5.52-4.48-10-10-10zm0 13c-1.66 0-3-1.34-3-3s1.34-3 3-3 3 1.34 3 3-1.34 3-3 3z">
                </path>
              </svg></a><a href="https://github.com/Bruno3869" class="group cursor-pointer" aria-label="Github"
              title="Github" target="_blank" rel="noreferrer"><svg stroke="currentColor" fill="none" stroke-width="2" viewBox="0 0 24 24"
                stroke-linecap="round" stroke-linejoin="round" height="1em" width="1em"
                xmlns="http://www.w3.org/2000/svg"
                class="text-xl text-fore-subtle group-hover:-translate-y-2 transition-transform">
                <path
                  d="M9 19c-5 1.5-5-2.5-7-3m14 6v-3.87a3.37 3.37 0 0 0-.94-2.61c3.14-.35 6.44-1.54 6.44-7A5.44 5.44 0 0 0 20 4.77 5.07 5.07 0 0 0 19.91 1S18.73.65 16 2.48a13.38 13.38 0 0 0-7 0C6.27.65 5.09 1 5.09 1A5.07 5.07 0 0 0 5 4.77a5.44 5.44 0 0 0-1.5 3.78c0 5.42 3.3 6.61 6.44 7A3.37 3.37 0 0 0 9 18.13V22">
                </path>
              </svg></a><a href="https://www.linkedin.com/in/brunocanini" class="group cursor-pointer" aria-label="Linkedin" title="Linkedin" target="_blank" rel="noreferrer"><svg
                stroke="currentColor" fill="none" stroke-width="2" viewBox="0 0 24 24" stroke-linecap="round"
                stroke-linejoin="round" height="1em" width="1em" xmlns="http://www.w3.org/2000/svg"
                class="text-xl text-fore-subtle group-hover:-translate-y-2 transition-transform">
                <path d="M16 8a6 6 0 0 1 6 6v7h-4v-7a2 2 0 0 0-2-2 2 2 0 0 0-2 2v7h-4v-7a6 6 0 0 1 6-6z"></path>
                <rect x="2" y="9" width="4" height="12"></rect>
                <circle cx="4" cy="4" r="2"></circle>
              </svg></a></span>
        </div>
      </div>
      <div class="float-sm-start float-md-end float-lg-end float-xl-end pt-3">
        <p class="text-muted">Powered by <a href="https://www.cosmicjs.com/" target="_blank" rel="noreferrer"
            class="ml-2 linkfooter"><svg version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg"
              xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" height="1.75em" width="1.75em"
              viewBox="0 0 98.1 118" style="enable-background:new 0 0 98.1 118;" xml:space="preserve">
              <style type="text/css">
                .sk0 {
                  fill: #FF3E00;
                }

                .sk1 {
                  fill: #FFFFFF;
                }
              </style>
              <path class="sk0" d="M91.8,15.6C80.9-0.1,59.2-4.7,43.6,5.2L16.1,22.8C8.6,27.5,3.4,35.2,1.9,43.9c-1.3,7.3-0.2,14.8,3.3,21.3
         c-2.4,3.6-4,7.6-4.7,11.8c-1.6,8.9,0.5,18.1,5.7,25.4c11,15.7,32.6,20.3,48.2,10.4l27.5-17.5c7.5-4.7,12.7-12.4,14.2-21.1
         c1.3-7.3,0.2-14.8-3.3-21.3c2.4-3.6,4-7.6,4.7-11.8C99.2,32.1,97.1,22.9,91.8,15.6" />
              <path class="sk1"
                d="M40.9,103.9c-8.9,2.3-18.2-1.2-23.4-8.7c-3.2-4.4-4.4-9.9-3.5-15.3c0.2-0.9,0.4-1.7,0.6-2.6l0.5-1.6l1.4,1
         c3.3,2.4,6.9,4.2,10.8,5.4l1,0.3l-0.1,1c-0.1,1.4,0.3,2.9,1.1,4.1c1.6,2.3,4.4,3.4,7.1,2.7c0.6-0.2,1.2-0.4,1.7-0.7L65.5,72
         c1.4-0.9,2.3-2.2,2.6-3.8c0.3-1.6-0.1-3.3-1-4.6c-1.6-2.3-4.4-3.3-7.1-2.6c-0.6,0.2-1.2,0.4-1.7,0.7l-10.5,6.7
         c-1.7,1.1-3.6,1.9-5.6,2.4c-8.9,2.3-18.2-1.2-23.4-8.7c-3.1-4.4-4.4-9.9-3.4-15.3c0.9-5.2,4.1-9.9,8.6-12.7l27.5-17.5
         c1.7-1.1,3.6-1.9,5.6-2.5c8.9-2.3,18.2,1.2,23.4,8.7c3.2,4.4,4.4,9.9,3.5,15.3c-0.2,0.9-0.4,1.7-0.7,2.6l-0.5,1.6l-1.4-1
         c-3.3-2.4-6.9-4.2-10.8-5.4l-1-0.3l0.1-1c0.1-1.4-0.3-2.9-1.1-4.1c-1.6-2.3-4.4-3.3-7.1-2.6c-0.6,0.2-1.2,0.4-1.7,0.7L32.4,46.1
         c-1.4,0.9-2.3,2.2-2.6,3.8s0.1,3.3,1,4.6c1.6,2.3,4.4,3.3,7.1,2.6c0.6-0.2,1.2-0.4,1.7-0.7l10.5-6.7c1.7-1.1,3.6-1.9,5.6-2.5
         c8.9-2.3,18.2,1.2,23.4,8.7c3.2,4.4,4.4,9.9,3.5,15.3c-0.9,5.2-4.1,9.9-8.6,12.7l-27.5,17.5C44.8,102.5,42.9,103.3,40.9,103.9" />
            </svg> SvelteKit</a> and <a href="https://kit.svelte.dev/" target="_blank" rel="noreferrer"
            class="ml-2 linkfooter"><svg version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg"
              xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" height="1.75em" width="1.75em"
              viewBox="0 0 52.6 50.5" enable-background="new 0 0 52.6 50.5" xml:space="preserve">
              <g>
                <path fill="#29ABE2" d="M20.4,17.3c1.3-1.1,2.5-2.2,3.8-3.2c-1-0.7-1.9-1.3-2.8-1.9c-1.1,0.9-2.2,1.9-3.3,2.9
             C19.2,15.5,20,16.3,20.4,17.3z" />

                <ellipse transform="matrix(1 2.637369e-03 -2.637369e-03 1 4.794111e-02 -4.540240e-02)" fill="#29ABE2"
                  cx="17.2" cy="18.2" rx="4.3" ry="4.1" />
                <path fill="#29ABE2" d="M21.1,41.2c-5.9,3.2-11,4-13.3,1.8c-3.2-3.1-0.3-11.2,6.7-19.4c-1.1-0.3-2-1.1-2.5-2.1
             C1.8,32.9-2.8,44.4,1.7,48.7c3.7,3.6,12.6,1.2,22.4-5.2C23.1,42.8,22.1,42,21.1,41.2z" />
                <path fill="#29ABE2" d="M28.1,13.9c0.3-1.1,1.1-2,2.1-2.5C18.4,1.7,6.4-2.6,1.9,1.7c-3.7,3.6-1.3,12.1,5.4,21.5
             c0.8-1,1.6-1.9,2.4-2.9C6.4,14.6,5.5,9.7,7.8,7.4C11.1,4.3,19.5,7.2,28.1,13.9z" />
                <path fill="#29ABE2" d="M36.1,16.7c-0.4,1-1.3,1.8-2.3,2.2c1.5,1.5,2.9,2.9,4.1,4.4c0.7-0.9,1.4-1.8,1.9-2.7
             C38.7,19.3,37.4,18,36.1,16.7z" />
                <path fill="#29ABE2" d="M33.7,20.7c2.4,0,4.3-1.8,4.3-4.1c0-2.3-1.9-4.1-4.3-4.1c-2.4,0-4.3,1.8-4.3,4.1
             C29.4,18.8,31.3,20.7,33.7,20.7z" />
                <path fill="#29ABE2" d="M44.8,7.5c3.2,3.1,0.3,11.2-6.7,19.4c1.1,0.3,2,1.1,2.5,2.1c10.1-11.4,14.7-23,10.2-27.3
             C47.1-1.8,38.2,0.6,28.5,7c1,0.7,2,1.5,3,2.3C37.4,6.1,42.5,5.3,44.8,7.5z" />
                <path fill="#29ABE2" d="M32.9,32.5c-1.5,1.4-3.1,2.7-4.6,3.9c1,0.7,1.9,1.3,2.9,1.9c1.4-1.1,2.7-2.3,4.1-3.6
             C34.2,34.3,33.4,33.5,32.9,32.5z" />

                <ellipse transform="matrix(-1 -2.754597e-03 2.754597e-03 -1 70.5604 64.8494)" fill="#29ABE2" cx="35.3"
                  cy="32.4" rx="4.3" ry="4.1" />
                <path fill="#29ABE2" d="M18.8,31.7c-1.5-1.5-2.9-2.9-4.1-4.4c-0.7,0.9-1.4,1.8-1.9,2.7c1.2,1.3,2.4,2.6,3.7,3.9
             C16.9,32.9,17.7,32.1,18.8,31.7z" />
                <path fill="#29ABE2" d="M45.3,27.4c-0.8,1-1.6,1.9-2.4,2.9c3.3,5.7,4.2,10.5,1.8,12.8c-3.3,3.1-11.7,0.2-20.3-6.4
             c-0.3,1.1-1.1,2-2.1,2.5c11.9,9.7,23.9,14.1,28.4,9.8C54.4,45.3,52,36.7,45.3,27.4z" />

                <ellipse transform="matrix(2.622796e-03 -1 1 2.622796e-03 -15.1309 52.7534)" fill="#29ABE2" cx="18.9"
                  cy="34" rx="4.1" ry="4.3" />
              </g>
            </svg> Cosmic</a></p>
      </div>
      <div class="float-start py-1 py-sm-3 py-lg-3 py-xl-3">
        <p class="text-muted">&copy; 2023 Brunocanini.com - Made with <a href="https://getbootstrap.com/" target="_blank" rel="noreferrer"
          class="ml-2 linkfooter"><svg xmlns="http://www.w3.org/2000/svg"
            height="1.25em" width="1.25em" fill="currentColor" class="bi bi-bootstrap-fill" viewBox="0 0 16 16">
            <g id="logo" fill="#7404f2">
              <path
                d="M6.375 7.125V4.658h1.78c.973 0 1.542.457 1.542 1.237 0 .802-.604 1.23-1.764 1.23H6.375zm0 3.762h1.898c1.184 0 1.81-.48 1.81-1.377 0-.885-.65-1.348-1.886-1.348H6.375v2.725z" />
              <path
                d="M4.002 0a4 4 0 0 0-4 4v8a4 4 0 0 0 4 4h8a4 4 0 0 0 4-4V4a4 4 0 0 0-4-4h-8zm1.06 12V3.545h3.399c1.587 0 2.543.809 2.543 2.11 0 .884-.65 1.675-1.483 1.816v.1c1.143.117 1.904.931 1.904 2.033 0 1.488-1.084 2.396-2.888 2.396H5.062z" />
            </g>
          </svg> Bootstrap</a></p>
      </div>
    </footer>
  </div>

<style>
  .text-muted {
  color: #6c757d!important;
  }
</style>